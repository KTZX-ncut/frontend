...
const onAbilityTimelineChanged = async params => {
  ...
  // courseGraphStore.setChart(0, courseGraphStore.charts[0].timelineData, [], [], []);
  if (currentRadarOption.value.options[params.currentIndex].series[0].data[0].value.length === 0) {
    const { code, msg } = await courseGraphStore.fetchCourseEvaluation(
      classroomId.value,
  ...
  currentRadarOption.value.baseOption.graphic = {
    ...
    top: 5,
    style: {
      text: `当前展示第${params.currentIndex + 1}次评价，已评价${courseGraphStore.totalTimes}次`
    }
  };
};
...
const onKWTimelineChanged = async params => {
  ...
  // 更新词云中的内容
  if (courseGraphStore.charts[1].options[currentWordIndex.value].series[0].data.length === 0) {
    courseGraphStore
...
const handleCellClick = async (row, column, cell) => {
  ...
  await initList();
  // 获取总评价次数
  const { code: timeCode, msg: timeMsg } = await courseGraphStore.fetchEvaluationTimes(
    courseId.value,
    classroomId.value
    // '292104772-9c9f88f0-7af9-438c-a117-e2efc2020b7b'
  );
  ...
  initChart(courseGraphStore.totalTimes);
  loading.value = false;
  console.log(courseGraphStore.charts[1].options[courseGraphStore.totalTimes - 1]);
  currentRadarOption.value.baseOption.graphic.style.text = `当前展示第${courseGraphStore.totalTimes}次评价，已评价${courseGraphStore.totalTimes}次`;
  currentWordOption.value.graphic[1] = {
    ...currentWordOption.value.graphic[1],
    style: {
      text: `当前展示第${courseGraphStore.totalTimes}次评价，已评价${courseGraphStore.totalTimes}次`
    }
  };
  currentTreeOption.value.baseOption.graphic[1] = {
    ...currentTreeOption.value.baseOption.graphic[1],
...
// 需要传递第几次作业，默认最新一次评价
const initChart = (num = courseGraphStore.totalTimes, isInit, which = 'all') => {
  // 渲染图表
  // 加载数据前清空图表
  radarInstance?.clear();
  wordInstance?.clear();
  ...
  if (which === 'RADAR') {
    courseGraphStore.updateCharts(num, isInit, 1);
    currentRadarOption.value = {
      ...radarOption(
        courseGraphStore.charts[0].timelineData,
        courseGraphStore.charts[0].options,
        courseGraphStore.charts[0].indicators
      )
    };
  } else if (which === 'WORD') {
    courseGraphStore.updateCharts(num, isInit, 2);
    try {
      const chartInstance = wordmapCmp.value?.getChartInstance();
      chartInstance.setOption(courseGraphStore.charts[1].options[currentWordIndex.value], {
        replaceMerge: 'series'
      });
    ...
    courseGraphStore.updateCharts(num, isInit, 3);
    currentTreeOption.value = {
      ...treeOption(
        courseGraphStore.charts[2].timelineData,
        courseGraphStore.charts[2].options,
        courseGraphStore.charts[2].response
      )
    };
  } else {
    ...
    currentWordOption.value = {
      ...wordOption(
        courseGraphStore.charts[1].timelineData,
        courseGraphStore.charts[1].options[courseGraphStore.totalTimes - 1]
      )
    };
    ...
    currentTreeOption.value = {
      ...treeOption(
        courseGraphStore.charts[2].timelineData,
        courseGraphStore.charts[2].options
        // courseGraphStore.charts[2].options[courseGraphStore.totalTimes - 1]
        // courseGraphStore.charts[2].options
      )
    };
    console.log(courseGraphStore.charts[2].options[courseGraphStore.totalTimes - 1]);
  }
};
...
onBeforeUnmount(() => {
  ...
  // 初始化pinia数据
  courseGraphStore.setChart(0);
  courseGraphStore.setChart(1);
  courseGraphStore.setChart(2);
});
</script>
