...
const fetchCourseList = async () => {
  await request.course
    .get('/coursemangt/courseresources')
    .then(res => {
      if (res.code === 200) {
...
const previewFile = async file => {
  ...
  }/coursemangt/courseresources/download/${encodeURIComponent(file.id)}`;
...
const downloadFile = file => {
  download(file, '/coursemangt/courseresources/download/');
};
...
//   let fileUrl = `${
//     request.course.defaults.baseURL
//   }/coursemangt/courseresources/download/${encodeURIComponent(file.id)}.${suffix}`;
//   request
//     .course({
//       url: fileUrl,
//       method: 'GET',
...
const deleteFile = async file => {
  try {
    ...
    const dotIndex = file.filename.lastIndexOf('.');
    let suffix = file.filename.substring(dotIndex + 1);
    let fileUrl = `/coursemangt/courseresources/delete/${encodeURIComponent(file.id)}.${suffix}`;
    await request.course
      .get(fileUrl)
      .then(res => {
        console.log('res', res);
