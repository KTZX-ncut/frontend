...
import { lessonPlanAgent } from '../agents';
...
export const dataExtractWorkflow = createWorkflowChain({
  ...
  .andAll({
    ...
    steps: [
      ...
      //               // 简单的进度日志
      //               console.log(
      //                 `[Task ${index}/${nodes.length}] ✅ 完成 (KU:${parsed.ku.length}, KW:${parsed.kw.length})`
      //               );
      ...
      //           .filter(
      //             (r): r is { status: 'success'; index: number; data: any } =>
      //               r.status === 'success'
      //           )
      //           .map(r => r.data);
      ...
      //         log?.info(
      //           `[Workflow] 最终产出: KU=${finalResult.ku.length}, KW=${finalResult.kw.length}, A=${finalResult.a.length}`
      //         );
